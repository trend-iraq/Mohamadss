...
const BUNNY_STORAGE_ZONE = 'trendiraq';
const BUNNY_STORAGE_HOST = 'https://storage.bunnycdn.com';
const BUNNY_STORAGE_PASSWORD = '925c56f0-933a-40ae-91b96d6d795c-b27a-4c42';
const BUNNY_CDN_URL = 'https://trendiraq.b-cdn.net';

// توليد اسم ملف فريد
function generateFileName(extension = 'jpg') {
  const rand = Math.random().toString(36).substring(2, 10);
  return `products/${Date.now()}-${rand}.${extension}`;
}

// رفع صورة (Blob) إلى Bunny.net، مع نظام احتياطي (fallback) لـ Base64 عند الفشل
async function uploadToBunny(blob, fileName) {
  try {
    ...
    console.warn('⚠️ فشل الرفع لـ Bunny.net، سيتم استخدام الطريقة الاحتياطية:', err);
    return null; // null يعني: استخدم Base64 كبديل
  }
}

// ضغط الصور تلقائياً ورفعها إلى Bunny.net (مع نظام احتياطي Base64 عند الفشل)
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = async () => {
        ...
        // تحويل الـ canvas إلى Blob (بدل Base64 مباشرة) لرفعه
        c.toBlob(async (blob) => {
          if (!blob) {
            // فشل غير متوقع بتحويل الصورة → استخدم Base64 كحل أخير
            return resolve(c.toDataURL('image/jpeg', quality));
          }
          const fileName = generateFileName('jpg');
          const cdnUrl = await uploadToBunny(blob, fileName);
          if (cdnUrl) {
            // نجح الرفع لـ Bunny.net ✅ - رابط قصير وسريع
            resolve(cdnUrl);
          } else {
            // فشل الرفع (مثلاً مشكلة CORS أو اتصال) → استخدم Base64 كحل أخير حتى لا يتعطل المتجر
            showToast('⚠️ تعذّر رفع الصورة لـ CDN، تم استخدام الطريقة القديمة', 'error');
  ...
  return `
    <div class="product-card">
      ...
      <div class="product-image-wrapper" data-product-id="${p.id}" data-action="view-product">
        <img class="product-image" src="${escapeHtml(mainImage)}" alt="${escapeHtml(p.name)}" loading="lazy" decoding="async" onerror="this.src='${placeholderImg}'" />
            ...
            <div class="wishlist-grid">
              ${wishlistProducts.map(p => `
                <div class="wishlist-item">
                  <img src="${escapeHtml((p.images?.[0]) || p.image || '')}" alt="${escapeHtml(p.name)}" loading="lazy" decoding="async" data-product-id="${p.id}" data-action="view-product" />
            ...
            ${state.cart.map(it => {
              ...
              return `
                <div class="cart-item ${it.bogo ? 'has-bogo' : ''}">
                  <img src="${escapeHtml(it.image)}" alt="${escapeHtml(it.name)}" loading="lazy" decoding="async" />
...
// معالجة الفيديو - رفع إلى Bunny.net (مع نظام احتياطي Base64 عند الفشل)
async function processVideo(file) {
  if (!file || !file.type.startsWith('video/')) throw new Error('ملف غير صالح');
  // الحد الأقصى للفيديو: 20 ميجا (أصبح ممكناً بعد الانتقال لـ Bunny.net بدل Base64)
  if (file.size > 20 * 1024 * 1024) {
    throw new Error('حجم الفيديو يجب أن يكون أقل من 20 ميجا');
  }
  
  const ext = (file.name.split('.').pop() || 'mp4').toLowerCase();
  const fileName = generateFileName(ext).replace('products/', 'videos/');
  const cdnUrl = await uploadToBunny(file, fileName);
  
  if (cdnUrl) {
    return cdnUrl;
  }
  
  // فشل الرفع (مثلاً مشكلة اتصال) → استخدم Base64 كحل أخير، لكن بحد أقصى أصغر لتفادي إبطاء الموقع
  if (file.size > 5 * 1024 * 1024) {
    throw new Error('تعذّر رفع الفيديو للخادم السريع، والحجم كبير جداً للطريقة الاحتياطية (الحد 5 ميجا)');
  }
  showToast('⚠️ تعذّر رفع الفيديو لـ CDN، تم استخدام الطريقة القديمة', 'error');
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
...
async function init() {
  ...
  onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
      ...
      // مستخدم عادي - إلغاء اشتراك الطلبات فقط (المنتجات والتقييمات مُحمّلة مسبقاً بالأسفل)
      state.view = 'store';
      unsubscribeAll();
      render();
    }
  ...
  // تحميل الإعدادات والأقسام والكوبونات والتقييمات بالتوازي (أسرع بكثير من التسلسلي)
  await Promise.all([
    loadSettings(),
    loadCategories(),
    loadCoupons(),
  ...
  // ابدأ الاشتراك بالمنتجات فوراً (فقط إن لم يكن أدمن مسجّل دخول بالفعل)
  if (!state.user) {
    subscribeProducts();
    subscribeReviews();
    render();
  ...
  // إخفاء شاشة التحميل فوراً بدون تأخير صناعي
